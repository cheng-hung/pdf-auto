...
def classify_acquisition_mode(distance, pdf_limit, saxs_limit):
    """Classify an acquisition using the configured detector-distance limits.

    Exact boundary values retain the behavior of the original workflow and are
    classified as ``"Unknown"``.
    """
    if distance < pdf_limit:
        return "PDF"
    if pdf_limit < distance < saxs_limit:
        return "XRD"
    if distance > saxs_limit:
        return "SAXS"
    return "Unknown"


class ImageDataConfig(ConfigParser):
...
class ImageData2D(ImageDataConfig):
    ...
    def acq_mode(self):
        try:
            ...
            acq_mode = classify_acquisition_mode(
                distance, self.PDF_limit, self.SAXS_limit
            )
    ...
    def stitch_images(self):
        """Stitch images according to their relative detector positions."""
        ...
        my_im1 = np.float32(
            getattr(self.run, self.stream_name[0]).read()[self.img_key].to_numpy()[0][0]
        )
        ...
        for i in range(self.num_positions):
            ...
            if "lambda" in self.img_key:
                ...
                img = np.float64(
                    self.run[self.stream_name[i]].read()[self.img_key].to_numpy()[0][0]
                )
        ...
        osetx_total = abs(
            round((sort_pos_x[-1] - sort_pos_x[0]) / self.pixel_size)
        )  ## pixels
        osety_total = abs(
            round((sort_pos_y[-1] - sort_pos_y[0]) / self.pixel_size)
        )  ## pixels
        ...
        my_imsum = (
            np.ones((x_size + osetx_total, y_size + osety_total, self.num_positions))
            * np.nan
        )
        x_sum_size = my_imsum.shape[0]  ## pixels
        y_sum_size = my_imsum.shape[1]  ## pixels

        ## Place an unshifted image at the center of the stitched canvas.
        base_start_x = (x_sum_size - x_size) // 2
        base_start_y = (y_sum_size - y_size) // 2
        ...
        for i in range(self.num_positions):
            ...
            start_x = base_start_x + x_offset  ## pixel
            end_x = start_x + x_size  ## pixel

            ## Define the broadcast range for y
            start_y = base_start_y + y_offset  ## pixel
            end_y = start_y + y_size  ## pixel

            my_imsum[start_x:end_x, start_y:end_y, i] = sort_my_im[:, :, i]
            my_imsum[start_x:end_x, start_y:end_y, i][sort_user_mask[:, :, i] == 1] = (
                np.nan
            )
    ...
    def subtract_dark(self):
        """Subtract the referenced dark image, when available."""
        ...
        try:
            ...
            dk_img = np.float32(dk_run.primary.read()[self.img_key].to_numpy()[0][0])
            # dk_img = 0.0
    ...
    def compute_processed_image(self):
        ...
        if self.stream_length == self.num_positions:
            ...
            if "pe" in self.detector:
                self.process_img = self.subtract_dark()

            else:
                if "lambda" in self.img_key:
                    self.process_img = np.float32(
                        getattr(self.run, self.stream_name[0])
                        .read()[self.img_key]
                        .to_numpy()[0]
                    )
                else:
