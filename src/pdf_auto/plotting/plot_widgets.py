...
class HistogramTuner(PlotTunerBase):
    def __init__(self, *args, q_array=None, histogram=True, **kwargs):
        ...
        gs = GridSpec(nrows=1, ncols=2, width_ratios=[1.0, 1.0])

        if self.histogram:
            gs.set_width_ratios([1.4, 1.0])
            self.ax1 = self.fig.add_subplot(gs[0, 0])
            self.im1 = is_q_space(
                self.q_array, self.img, self.ax1, self.vmax, self.vmin, self.aspect
            )
            ...
            self.ax2.hist(
                self.img.flatten(), bins=1000, log=True, histtype="stepfilled"
            )
...
class TwoSubTuner(PlotTunerBase):
    def __init__(
        self,
        *args,
        q_array=None,
        ...
        **kwargs,
        ...
        if self.data is not None:
            ...
            self.ax2 = self.fig.add_subplot(gs[0, 1])
            self.ax2.plot(
                self.data[0], self.data[1], label=self.sample_name, color=self.color_str
            )
            # self.ax2.set_title('Histogram of pixel intensities')
            ...
            self.slider_iq = Slider(
                self.slider_ax_, "q_range", self.q_binned[0] - 1, self.q_binned[-1] + 1
            )
    ...
    def update_iq(self, val):
        ...
        if self.pyfai_split:
            ...
            circle = circle_coords(
                center=[self.center_x, self.center_y], radius=self.q_to_r()
            )
...
class ThreeSubTuner(PlotTunerBase):
    def __init__(
        self,
        *args,
        unrolled_array=None,
        ...
        **kwargs,
        ...
        gs = GridSpec(nrows=2, ncols=2, width_ratios=[1.0, 1.0])

        if (self.data is not None) and (self.unrolled_array is not None):
            gs.set_width_ratios([1.0, 1.2])
            gs.set_height_ratios([1.2, 1.0])
            ## plot img array at the top left corner
            self.ax1 = self.fig.add_subplot(gs[:, 0])
            self.im1 = is_q_space(
                None, self.img, self.ax1, self.vmax, self.vmin, self.aspect
            )
            ...
            ## Add slider
            self.q_binned = bin_ndarray(
                self.data[0], new_shape=(self.unrolled_array.shape[1],)
            )
            self.fig.subplots_adjust(bottom=0.15)
            self.slider_ax_ = self.fig.add_axes([0.15, 0.05, 0.65, 0.03])
            self.slider_iq = Slider(
...
plot_tuner_base = PlotTunerBase
histogram_tuner = HistogramTuner
TwoSub_tuner = TwoSubTuner
ThreeSub_tuner = ThreeSubTuner
